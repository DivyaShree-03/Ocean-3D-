...
export function sampleCurrentField(lon: number, lat: number): CurrentVector {
  ...
  // Tangential velocity vector for clockwise rotation (u = +dz, v = -dx)
  const uBob = +bobDz * bobFactor;
  const vBob = -bobDx * bobFactor;
  ...
  const uArab = +arabDz * arabFactor;
  const vArab = -arabDx * arabFactor;
  ...
  const vEq = 0.35 * Math.sin((lon - 75.0) * 0.25) * eqFactor;

  // 4. Broad North Equatorial West-to-East Drift (background flow)
  const uDrift = 0.22 + 0.15 * Math.cos(lat * 0.12);
  const vDrift = 0.08 * Math.sin(lon * 0.1);
